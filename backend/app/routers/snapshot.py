...
ftc_router = APIRouter()


async def _build_ftc_snapshot(event_key: str) -> dict:
    """Build a full FTC event snapshot using the FTC service layer."""
    from ..services import ftc_event_service

    info_coro = ftc_event_service.get_event_info(event_key)
    teams_coro = ftc_event_service.get_event_teams_with_stats(event_key)
    matches_coro = ftc_event_service.get_all_matches(event_key)
    alliances_coro = ftc_event_service.get_alliances(event_key)
    playoffs_coro = ftc_event_service.get_playoff_matches(event_key)

    info, teams, matches_data, alliances, playoffs = await asyncio.gather(
        info_coro,
        teams_coro,
        _safe(matches_coro),
        _safe(alliances_coro),
...
async def _ftc_ingest_and_build(event_key: str) -> dict:
    """Cold-miss path: ensure FTC event data exists, then build snapshot.

    If the warm ftc_event_sync worker has already populated Supabase (which
    it does every 120 s for all ongoing events) the ingest step is skipped.
    Either way, the event is registered for live match polling so the 5-second
    poller keeps scores up to date while the caster has it open.
    """
    from ..services import ingestion_service
    if not await ingestion_service.is_ftc_ingested(event_key):
        # Cold start — worker hasn't run yet; fetch everything now
        await ingestion_service.ingest_ftc_event(event_key)
    else:
        # Data already in Supabase; just register for live polling
        try:
            from ..workers.ftc_match_poller import add_watched_ftc_event
            add_watched_ftc_event(event_key)
        except Exception:
            pass
    payload = await _build_ftc_snapshot(event_key)
    _write_snapshot(event_key, payload)
    return payload


async def _ftc_background_rebuild(event_key: str) -> None:
    """Rebuild FTC snapshot in background (stale-while-revalidate)."""
    try:
        payload = await coalesce(
            f"snapshot_build:{event_key}", _build_ftc_snapshot, event_key,
        )
        _write_snapshot(event_key, payload)
        log.info("Background rebuild of FTC %s snapshot complete", event_key)
    except Exception:
        log.warning("Background rebuild of FTC %s snapshot failed", event_key, exc_info=True)


@ftc_router.get("/{event_key}/snapshot")
async def ftc_event_snapshot(event_key: str):
    """Return a cached full FTC event payload, ingesting on first request.

    On cold miss the server fetches all event data from the FTC Events API,
    persists it to Supabase, and registers the event for live polling if it
    is currently ongoing — matching the FRC snapshot / ingest flow exactly.
    """
    ...
    if cached and not fresh:
        asyncio.create_task(_ftc_background_rebuild(event_key))
        cached["_cached"] = True
        cached["_stale"] = True
        return cached

    # Cold miss — single-flight ingest + build
    try:
        cached_again, _ = _read_snapshot(event_key)
        if cached_again:
            cached_again["_cached"] = True
        ...
        payload = await coalesce(
            f"snapshot_build:{event_key}", _ftc_ingest_and_build, event_key,
        )
        ...
        log.exception("FTC snapshot build failed for %s", event_key)
        raise_api_error(
            e,
            fallback_detail=f"Could not build FTC snapshot for '{event_key}'.",
        )
